...
- Payments: Stripe **Embedded Checkout** on `/settings/credits`. Server action creates a Checkout Session with `ui_mode: 'embedded_page'` (renamed from `'embedded'` in stripe-node v21+). Webhook is a **Supabase Edge Function** at `POST <supabase>/functions/v1/stripe-webhook` listening for `checkout.session.completed`, idempotent on `stripe_session_id`. (Was a Next.js route — deleted; the Edge Function runs next to the DB with one env surface.) See `docs/integrations/STRIPE.md`.
- Local Stripe webhook = two processes: `supabase functions serve stripe-webhook --no-verify-jwt --env-file .env.local` AND `stripe listen --forward-to localhost:54321/functions/v1/stripe-webhook`. Without `--forward-to`, events are observed but not delivered (most common cause of "Stripe says success, DB has nothing").
- `config.toml` has `[functions.stripe-webhook] verify_jwt = false` — Stripe sends no Supabase JWT; signature is verified inside the function.
...
- The Edge Function uses the auto-injected `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (Supabase provides these to every function). `SUPABASE_SECRET_KEY` / `createServiceClient` are gone — they only served the deleted Next route. Stripe secrets reach the function via `--env-file .env.local` (local) or `supabase secrets set` (hosted).
